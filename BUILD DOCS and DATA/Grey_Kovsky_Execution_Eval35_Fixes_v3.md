...
| `src/acp/JobRouter.ts` | plugin-wpv | Fix 2: "Unknown" fallback, discovery guard removal, Fix 8: AbortController in withTimeout |
| `src/verification/ClaimExtractor.ts` | plugin-wpv | Fix 4: min text threshold (200 chars) |
| `src/discovery/CryptoContentResolver.ts` | plugin-wpv | Fix 5: SPA early signal (`PLAYWRIGHT_FAILED` diagnostic), Fix 8: AbortSignal threading |
...
| `src/types.ts` | plugin-wpv | Fix 8: IContentResolver signal parameter |
