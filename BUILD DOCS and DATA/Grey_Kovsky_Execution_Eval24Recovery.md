# Kovsky Execution Plan — Unified Eval 24 Recovery + Plugin Trim
...
7. Concatenate: landing page text + sub-page texts, separated by `\n\n--- [Section: {page title or path}] ---\n\n`
...
private extractLinks(html: string, origin: string): string[] {
  ...
  while ((match = linkPattern.exec(html)) !== null) {
    try {
      ...
      if (href.split('#')[0] === origin) continue;
      // Deduplicate
...
private scoreLink(href: string): number {
  ...
  const skip = [
    ...
    'discord', 'telegram', 'medium.com', 'changelog', 'migration',
  ];
...
### 1D. Persistence Hook Point (for next phase)
...
### Issue B: DocsSiteCrawler re-fetching introduces duplication risk

**Problem:** The landing page is fetched twice — once by FetchContentResolver (for content) and once by DocsSiteCrawler (for links). The landing page text appears in both the initial content and potentially in a sub-page.

**Resolution:** DocsSiteCrawler should NOT include the landing page text in its concatenated output. It starts concatenation from sub-pages only. CryptoContentResolver prepends the original landing page text if needed, or the crawled content replaces it entirely (since sub-pages should contain more comprehensive content than the intro).

**Decision:** Crawled content REPLACES landing page content entirely when `crawled.text.length > content.text.length * 1.5`. The 1.5x threshold ensures we only replace when the crawl found substantially more. This is already in the integration code above.
...
*Forces-approved. Implement in order: Phase 0 → 1 → 2 → 3 → 4 → 5.*
