# ACP v2 SDK Migration Plan — Whitepaper Grey (v5)

> **Source:** Forces v1 → Kovsky v1 review → Forces v2 → Kovsky v2 review → Kovsky v3 → Forces v4 → Kovsky v4 SDK verification → Kovsky v5
> **Date:** 2026-04-11
  ...
  try {
    ...
    for (const ref of jobRefs) {
      try {
        ...
        if (full) {
          // api.getJob() returns OffChainJob (budget is string | null).
          // AcpJob.fromOffChain() wraps it as AssetToken, but we use the raw API here.
          const budgetStr = full.budget;
          jobs.push({
            ...
            price: budgetStr ? Number(budgetStr) : 0,
  ...
  try {
    ...
    if (typeof (api as any).getCompletedJobs === 'function') {
      ...
      return jobs.map((j: any) => {
        // budget may be string (OffChainJob), AssetToken (AcpJob), or number — parse defensively
        const rawBudget = j.budget;
        const price = typeof rawBudget === 'number' ? rawBudget
          : typeof rawBudget === 'string' ? Number(rawBudget)
...
## v5 Verification Checklist (all verified against actual SDK type definitions)
...
- [x] `OffChainJob.budget`: `string | null` — confirmed (raw API returns string)
