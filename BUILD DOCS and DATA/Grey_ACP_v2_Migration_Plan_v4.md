# ACP v2 SDK Migration Plan — Whitepaper Grey (v4)

> **Source:** Forces v1 → Kovsky v1 review → Forces v2 → Kovsky v2 thorough review → Kovsky v3 rewrite → Forces v4 review (5 issues fixed)
> **Date:** 2026-04-11
...
> 2. **Runtime bug:** `getActiveJobs()` returns `{ chainId, onChainJobId }[]` only — not full job objects. Kov's v3 mapped `j.id`, `j.status`, `j.clientAddress` which would all be `undefined`. Fixed to call `api.getJob()` per job.
